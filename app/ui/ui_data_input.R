data_input_ui <- function() {
  sidebar(
    title = "Data Input",
    customFileInput("crispr_file", "Upload CRISPR Data (CSV)", accept = c(".csv"), multiple = TRUE),
    customFileInput("pisa_file", "Upload PISA File (CSV, Optional)", accept = c(".csv"), multiple = TRUE),
    customFileInput("meta_file", "Upload Metadata File (CSV)", accept = c(".csv"), multiple = TRUE),
